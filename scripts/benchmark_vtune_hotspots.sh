...
sudo -E "$VTUNE_BIN" -collect hotspots -result-dir "$RES_DIR" env TMP_DIR="$TMP_DIR" ./scripts/interactive_runner.sh "$APP_BIN"
...
echo "👉 Pour explorer visuellement le code et l'assembleur : task profile-vtune-gui (ou vtune-gui $RES_DIR)"
