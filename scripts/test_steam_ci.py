...
def test_interactive_window_presentation(target_exe: Path) -> None:
    ...
    subprocess.run("WINEDEBUG=-all wineserver -k 2>/dev/null || true", shell=True)
    subprocess.run("WINEDEBUG=-all wineserver -w 2>/dev/null || true", shell=True)
    ...
    run_cmd_str = (
        ...
        f"{wait_win_cmd}; sleep 5; {capture_subcmd}; "
