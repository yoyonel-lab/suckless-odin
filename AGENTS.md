...
2. **Utiliser systématiquement `task <nom>`** :
   - Tests : `task test`, `task test-unit`, `task test-cli`, `task test-shader`, `task test-win`
...
## 🚫 RÈGLE ABSOLUE : RESTRICTION DES TESTS XVFB / LLVMPIPE EN LOCAL (PRÉSERVATION CPU)

1. **Pas d'exécution systématique de `task test-gl-xvfb` ni Xvfb en local** : L'émulation logicielle OpenGL CPU (`llvmpipe` / Mesa software rasterizer sous Xvfb) sature et surchauffe intensivement les cœurs CPU de la machine hôte.
2. **Usage exclusif en cas de régression CI/CD** : `task test-gl-xvfb` ne doit être exécuté en local **QUE pour reproduire et valider un bug spécifiquement détecté et remonté par un job de la CI/CD GitHub Actions**.
3. **Validation locale courante légère obligatoire** : Se limiter strictement à `task lint`, `task test-unit`, `task test-shader` pour valider le code au quotidien.
4. **Tests graphiques automatisés réservés à la CI / Docker** : Les tests offscreen lourds restent cantonnés aux environnements de CI automatisés en routine.
