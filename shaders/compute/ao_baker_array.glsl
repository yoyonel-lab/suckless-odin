...
// -------------------------------------------------------------------
// Ground Truth Ambient Occlusion Direct In-VRAM Batch Compute Shader
// Computes high-precision cosine-weighted raytraced AO maps directly
// into slices of a GL_TEXTURE_2D_ARRAY (Zero-Disk, Zero-Copy).
// -------------------------------------------------------------------

layout(local_size_x = 16, local_size_y = 16, local_size_z = 1) in;

// Output: 2D Texture Array (GL_R8, width x height x 100 slices)
layout(r8, binding = 0) uniform writeonly image2DArray u_ao_array_out;
...
uniform int u_start_sphere_idx;
...
void main()
{
    ...
    int target_sphere = u_start_sphere_idx + int(gl_GlobalInvocationID.z);
    if (target_sphere >= u_total_spheres || target_sphere < 0) {
        return;
    }
    ...
    imageStore(u_ao_array_out, ivec3(pixel, target_sphere), vec4(ao, 0.0, 0.0, 0.0));
}
