...
// applies 3x3 neighborhood bounding box clamping, and performs Exponential Moving Average (EMA).
...
layout(binding = 0) uniform sampler2D u_current_shadow;  // Current frame raw shadow visibility (RGBA16F)
layout(binding = 1) uniform sampler2D u_history_shadow;  // Previous frame accumulated shadow buffer (RGBA16F)
layout(binding = 2) uniform sampler2D u_current_depth;   // Current frame linear depth in meters (R32F)
layout(binding = 3) uniform sampler2D u_history_depth;   // Previous frame linear depth in meters (R32F)
...
uniform int   u_taa_mode;          // 0: Off (Raw Passthrough), 1: Simple EMA Blend, 2: TAA Reprojection
uniform float u_alpha;             // Current frame blend weight (default 0.15, range 0.01..1.00)
uniform float u_depth_threshold;   // Disocclusion depth tolerance in meters (default 0.30)
uniform bool  u_clamping_enabled;  // 3x3 neighborhood bounding box clamping
uniform bool  u_history_valid;     // False on first frame / reset

void main()
{
    vec4 curr_val = texture(u_current_shadow, TexCoords);

    // Mode 0: Off / Raw Passthrough
    if (u_taa_mode == 0) {
        FragColor = curr_val;
    ...
    if (u_taa_mode == 1) {
        ...
        vec4 prev_val = texture(u_history_shadow, TexCoords);
        if (u_clamping_enabled) {
            vec2 texel = 1.0 / vec2(textureSize(u_current_shadow, 0));
            vec4 box_min = curr_val;
            vec4 box_max = curr_val;
            for (int dy = -1; dy <= 1; ++dy) {
                for (int dx = -1; dx <= 1; ++dx) {
                    vec4 n = texture(u_current_shadow, TexCoords + vec2(dx, dy) * texel);
                    box_min = min(box_min, n);
                    box_max = max(box_max, n);
                }
            }
            prev_val = clamp(prev_val, box_min, box_max);
        }
        FragColor = mix(prev_val, curr_val, clamp(u_alpha, 0.01, 1.0));
    ...
    vec4 prev_val = offscreen ? curr_val : texture(u_history_shadow, prev_uv);

    // 3. 3x3 Neighborhood Clamping to eliminate ghosting on moving shadows/objects
    if (u_clamping_enabled) {
        vec2 texel = 1.0 / vec2(textureSize(u_current_shadow, 0));
        vec4 box_min = curr_val;
        vec4 box_max = curr_val;
    ...
    FragColor = mix(prev_val, curr_val, clamp(effective_alpha, 0.0, 1.0));
