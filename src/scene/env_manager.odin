...
env_manager_set_transition_state :: proc(mgr: ^Env_Manager, new_state: Transition_State) {
	...
	log.log_info("scene.env", "Transition state: %v -> %v (previous duration: %.3fs)",
...
env_manager_set_ibl_state :: proc(mgr: ^Env_Manager, new_state: IBL_State) {
	...
	log.log_info("scene.env", "IBL state: %v -> %v (previous duration: %.3fs)",
...
env_manager_trigger_transition :: proc(mgr: ^Env_Manager, path: string) -> bool {
	...
	log.log_info("scene.env", "Transition triggered: %s", path)
	return true
}
...
env_manager_trigger_initial :: proc(mgr: ^Env_Manager, path: string) {
	...
	log.log_info("scene.env", "Initial env load triggered: %s", path)
}
...
env_manager_poll_loader :: proc(mgr: ^Env_Manager) {
	...
	case .Ready:
		...
		log.log_info("scene.env", "Async load complete, starting IBL pipeline")
...
env_manager_ibl_upload_texture :: proc(mgr: ^Env_Manager, scene: ^Scene) {
	...
	if mgr.recycled_hdr_tex != 0 {
		...
		if rec_w == w && rec_h == h {
			...
			log.log_info("scene.env", "IBL: Reusing recycled environment texture (%dx%d)", w, h)
			reused = true
		}
	}
	...
	if !reused && mgr.is_first_load && scene.env_texture.id != 0 {
		if scene.env_texture.width == w && scene.env_texture.height == h {
			...
			gl.BindTexture(gl.TEXTURE_2D, mgr.pending_hdr_tex)
			log.log_info("scene.env", "IBL: Eagerly took initial scene environment texture (%dx%d)", w, h)
			reused = true
		}
	}
	...
	if !reused {
		...
		log.log_info("scene.env", "IBL: Allocated new environment texture (%dx%d)", w, h)
	}
	...
	log.log_info("scene.env", "IBL: Texture progressive upload started (%dx%d, Persistent Ring PBO DMA active)", w, h)
}
...
env_manager_ibl_upload_progressive :: proc(mgr: ^Env_Manager) {
	...
	if mgr.ibl_current_slice >= mgr.ibl_total_slices {
		...
		mgr.ibl_current_slice = 0
		log.log_info("scene.env", "IBL: Progressive upload complete, proceeding to Generate_Mipmaps")
	}
}
...
env_manager_ibl_generate_mipmaps :: proc(mgr: ^Env_Manager) {
	...
	log.log_info("scene.env", "IBL: Mipmaps generated, async luminance readback triggered")
}
...
env_manager_ibl_luminance :: proc(mgr: ^Env_Manager) {
	...
	log.log_info("scene.env", "IBL: Luminance threshold = %.2f", mgr.ibl_clamp_threshold)
}
...
env_manager_ibl_specular_init :: proc(mgr: ^Env_Manager) {
	...
	log.log_info("scene.env", "IBL: Reusing pooled specular texture (slot %d, zero alloc), starting progressive mips", pending_idx)
}
...
env_manager_process_specular_slice :: proc(mgr: ^Env_Manager, ibl: ^rendering.IBL_Resources) {
	...
	if mgr.ibl_current_mip >= mgr.ibl_total_mips {
		log.log_info("scene.env", "IBL: Specular complete")
...
env_manager_start_irradiance :: proc(mgr: ^Env_Manager) {
	...
	log.log_info("scene.env", "IBL: Specular complete, starting irradiance on pooled texture (slot %d, %d slices)", pending_idx, mgr.compute_tuning.slicing.irdiff_slices)
}
...
env_manager_process_irradiance_slice :: proc(mgr: ^Env_Manager, ibl: ^rendering.IBL_Resources) {
	...
	if actual_lines <= 0 {
		...
		if mgr.ibl_current_slice >= mgr.ibl_total_slices {
			...
			log.log_info("scene.env", "IBL: Irradiance complete")
		}
		return
	}
...
env_manager_capture_snapshot :: proc(mgr: ^Env_Manager, width, height: i32) {
	...
	log.log_info("scene.env", "Env: Captured snapshot (%dx%d)", width, height)
}
...
env_manager_swap_textures :: proc(mgr: ^Env_Manager, scene: ^Scene) {
	...
	log.log_info("scene.env", "Environment textures swapped simultaneously (pool slot %d active)", mgr.pool_active_idx)
