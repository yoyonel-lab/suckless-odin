...
env_thumbnail_ensure_loaded :: proc(thumb: ^Env_Thumbnail) {
	...
	if data == nil {
		log.log_warning("render.texture", "Failed to load thumbnail HDR: %s", thumb.path)
		return
	}
	...
	log.log_debug("render.texture", "Lazy-loaded thumbnail on demand: %s (%dx%d)", thumb.filename, thumb.width, thumb.height)
}
...
env_thumbnails_init :: proc(mgr: ^Env_Thumbnail_Manager, hdr_paths: []string) {
	...
	log.log_info("render.texture", "Registered %d environment map thumbnails (Lazy Load)", len(mgr.thumbnails))
}
