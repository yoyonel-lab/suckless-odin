...
// Phase 3: Volumetric Lighting & Raymarching State
Volumetric_Renderer :: struct {
	enabled:                bool,
	fbo:                    u32,
	raw_tex:                u32, // GL_RGBA16F, W/2 x H/2 (RGB: In-Scattering, A: Transmittance)
	...
	scattering_coeff:       f32,  // Scattering coefficient sigma_s (default 0.20, range 0.01..2.0)
	extinction_coeff:       f32,  // Extinction coefficient sigma_t (default 0.05, range 0.0..1.0)
	anisotropy_g:           f32,  // Henyey-Greenstein eccentricity g (default 0.40, range -0.90..+0.90)
	...
	preview_mode:           i32, // 0: In-Scattering, 1: Transmittance, 2: False Color Heatmap
...
volumetric_create :: proc(vr: ^Volumetric_Renderer, full_width, full_height: i32) -> bool {
	...
	log.log_info("suckless-odin.volumetric", "Volumetric renderer created (%dx%d -> %dx%d)", vr.full_width, vr.full_height, vr.width, vr.height)
	return true
}

@(private)
create_volumetric_fbo :: proc(vr: ^Volumetric_Renderer) -> bool {
	// Raw volumetric HDR texture (GL_RGBA16F)
	...
	if status != gl.FRAMEBUFFER_COMPLETE {
		log.log_error("suckless-odin.volumetric", "Volumetric FBO incomplete: 0x%X", status)
...
// Executes the Volumetric Raymarching pass
volumetric_render :: proc(
	...
	dbg.push_group("Volumetric_Raymarching_Pass")
...
volumetric_update_preview :: proc(vr: ^Volumetric_Renderer) {
	...
	gl.Uniform1i(vr.preview_loc_mode, vr.preview_mode)
...
volumetric_composite_to_scene :: proc(vr: ^Volumetric_Renderer, target_fbo: u32, width, height: i32) {
	if !vr.enabled || !vr.composite_in_scene || vr.composite_program == 0 || vr.raw_tex == 0 do return
